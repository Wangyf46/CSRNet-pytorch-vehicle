...
def get_interpolation(gt_density_map):
        shape1 = int(gt_density_map.shape[1] / 8)           # w
        shape0 = int(gt_density_map.shape[0] / 8)           # h
        gt_density_map_interpolation = cv2.resize(gt_density_map, (shape1, shape0)) * 64.0
        #et_density_map_interpolation = cv2.resize(et_density_map, (shape1, shape0)) / 64.0
        gt_density_map_interpolation = 255 * gt_density_map_interpolation / np.max(gt_density_map_interpolation)
        return gt_density_map_interpolation

def get_quality_psnr(gt_density_map, et_density_map):
    if gt_density_map.shape[1] != et_density_map.shape[1]:
       gt_density_map = get_interpolation(gt_density_map)
    diff = np.sum((abs(gt_density_map - et_density_map)) ** 2)
    mse = diff / gt_density_map.size
    PSNR = 20 * (math.log(10, 255 / np.sqrt(mse)))
    return PSNR, gt_density_map
